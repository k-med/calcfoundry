...
date: 2026-05-26
categories: ["Finance"]
summary: "Calculate your periodic mortgage or loan payments, see the impact of extra payments, and estimate how much interest you can save."
math: true
...
<input type="number" id="extra_payment" placeholder="e.g. 200" value="0">
    ...
    <button onclick="calculate_mortgage_loan_calculator()">Calculate Payments</button>
  ...
  <div class="calc-history">
    ...
    <ul id="history_list_mortgage_loan_calculator"></ul>
    
    <div style="display:flex; gap:10px; margin-top:10px;">
        <button onclick="downloadHistory_mortgage_loan_calculator()" class="btn-small" style="flex:1;">Save</button>
        <button onclick="clearHistory_mortgage_loan_calculator()" class="btn-small" style="flex:1;">Clear</button>
    </div>
  </div>
</div>

<script>
    const STORAGE_KEY_mortgage_loan_calculator = "calcfoundry_history_mortgage_loan_calculator"; 

    window.onload = function() { renderHistory_mortgage_loan_calculator(); };

    function calculate_mortgage_loan_calculator() {
    ...
    if (isNaN(P) || P <= 0 || isNaN(r_annual) || r_annual < 0 || isNaN(t_years) || t_years <= 0) {
        ...
        if(historySummary) addToHistory_mortgage_loan_calculator(historySummary);
    }

    function addToHistory_mortgage_loan_calculator(item) {
        let history = JSON.parse(localStorage.getItem(STORAGE_KEY_mortgage_loan_calculator)) || [];
        if (history.length === 0 || history[0] !== item) {
            history.unshift(item);
            if (history.length > 10) history.pop();
            localStorage.setItem(STORAGE_KEY_mortgage_loan_calculator, JSON.stringify(history));
            renderHistory_mortgage_loan_calculator();
        }
    }

    function renderHistory_mortgage_loan_calculator() {
        const list = document.getElementById('history_list_mortgage_loan_calculator');
        const history = JSON.parse(localStorage.getItem(STORAGE_KEY_mortgage_loan_calculator)) || [];
        list.innerHTML = history.map(item => `<li>${item}</li>`).join('');
    }

    function clearHistory_mortgage_loan_calculator() {
        localStorage.removeItem(STORAGE_KEY_mortgage_loan_calculator);
        renderHistory_mortgage_loan_calculator();
    }

    function downloadHistory_mortgage_loan_calculator() {
        const history = JSON.parse(localStorage.getItem(STORAGE_KEY_mortgage_loan_calculator)) || [];
        if (history.length === 0) {
        ...
        let content = "CalcFoundry - Mortgage Loan Calculator History\n";
        ...
        a.href = url;
        a.download = "Mortgage_Loan_Calculator_History.txt";
